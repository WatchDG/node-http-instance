...
function _request(protocol: string, options: http.RequestOptions | https.RequestOptions): Promise<ResultOK<HttpResponse> | ResultFAIL<Error>> {
    const fn: (options: http.RequestOptions | https.RequestOptions, callback?: (response: http.IncomingMessage) => void) => http.ClientRequest = protocol === 'http:' ? http.request : https.request;
    return new Promise(resolve => {
        try {
            const request = fn(options, response => {
                ...
                response.on('data', chunk => buffer = Buffer.concat([buffer, chunk], Buffer.byteLength(buffer) + Buffer.byteLength(chunk)));
                response.on('end', () => {
...
export class HttpInstance {
    ...
    get(path: string) {
        ...
        return _request(this.url.protocol, Object.assign(this.options, {
            method: 'GET',
            path: totalPath
        }));
    }
